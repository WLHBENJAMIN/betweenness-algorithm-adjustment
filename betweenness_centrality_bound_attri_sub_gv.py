...
def betweenness_centrality_radius_attribute_subset_gv(G, 
                                                      weight=None, 
                                                      radius=None, 
                                                      attribute=None, 
                                                      sources=None, 
    ...
    for s in tqdm(sources, desc="Processing sources", unit="source"):
        S, P, sigma, D = _single_source_dijkstra_path_basic_radius(G, s, weight, radius)
        betweenness, _ = _accumulate_attribute_subset(G, betweenness, 
                                                      S, P, sigma, D, s, 
                                                      attribute, targets, 
                                                      alpha, beta)
    return betweenness
...
def _accumulate_attribute_subset(G, betweenness, 
                                 S, P, sigma, D, s, 
                                 attribute, targets, 
                                 alpha, beta):
    ...
    while S:
        ...
        if w in target_set:
            ...
            attribute_w_gravity = attribute_w ** alpha / math.exp(beta * D[w])
            attribute_s = G.nodes[s][attribute] if attribute is not None else 1
            term = attribute_s * attribute_w_gravity
            coeff = (term + delta[w]) / sigma[w]
        else:
